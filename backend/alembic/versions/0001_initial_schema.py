...
def upgrade() -> None:
    ...
    if "customers" not in existing_tables:
        op.create_table(
            ...
            sa.Column("external_customer_id", sa.String(255), unique=True, nullable=True),
    ...
    if "payments" not in existing_tables:
        op.create_table(
            "payments",
            sa.Column("id", sa.Uuid(as_uuid=True), primary_key=True),
            sa.Column("external_payment_id", sa.String(255), unique=True, nullable=True),
    ...
    if "recovery_opportunities" not in existing_tables:
        ...
        op.create_index("ix_recovery_opportunities_payment_id", "recovery_opportunities", ["payment_id"], unique=True)
        op.create_index("ix_recovery_opportunities_status", "recovery_opportunities", ["status"])

    # 4. recovery_attempts table
    if "recovery_attempts" not in existing_tables:
        op.create_table(
            "recovery_attempts",
            sa.Column("id", sa.Uuid(as_uuid=True), primary_key=True),
        ...
        op.create_index("ix_recovery_attempts_opportunity_id", "recovery_attempts", ["opportunity_id"])
