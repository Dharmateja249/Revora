...
class PolicyValidator:
    ...
    def validate_decision(
        ...
        if not isinstance(candidate_action, RecoveryAction):
            raise TypeError(f"Expected candidate_action to be RecoveryAction, got {type(candidate_action).__name__}")
        if not isinstance(policy_context, RecoveryPolicyContext):
            raise TypeError(f"Expected policy_context to be RecoveryPolicyContext, got {type(policy_context).__name__}")

        applied_ids: Tuple[str, ...] = tuple(r.policy_id for r in policy_context.applicable_rules)
        ...
        violated_tuple = tuple(violated_rules) if violated_rules else ("POLICY_ACTION_NOT_PERMITTED",)
        ...
        if policy_context.mandatory_fallback_action is not None and (
            ...
            non_stop = [a for a in policy_context.allowed_actions if a != RecoveryAction.NO_ACTION]
